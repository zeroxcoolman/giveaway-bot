...
# --- Fertilizers ---
fertilizers = {
    "Speed Boost": {"effect": "reduce_time", "amount": 0.5, "cost": 15, "description": "Reduces grow time by 50%"},
    "Mutation Booster": {"effect": "increase_mutation", "amount": 2.0, "cost": 30, "description": "Doubles mutation chance"}
}
user_fertilizers = defaultdict(lambda: {"Speed Boost": 0, "Mutation Booster": 0})

# --- Season Boosts ---
current_season = {"name": "Spring", "boosted_seeds": ["Strawberry", "Ember Lily"]}

# --- Global Events ---
plant_events = [
    {"name": "Blight", "effect": "delay", "chance": 0.05, "delay": 180},
    {"name": "Rain", "effect": "speed", "chance": 0.07, "multiplier": 0.85}
]
current_plant_event = random.choice([e for e in plant_events if random.random() < e["chance"]] + [None])

# --- Quests ---
user_quests = defaultdict(lambda: {"daily_done": False, "weekly_progress": 0})
daily_quest = {"desc": "Grow 1 seed (any)", "reward": 2}
weekly_quest = {"desc": "Grow 5 Mythic/Legendary", "target": 5, "reward": 20}

# --- Lootboxes ---
lootbox_types = {
    "Common": {"seeds": ["Strawberry", "Potato", "Carrot"], "cost": 10},
    "Rare": {"seeds": ["Bamboo", "Cauliflour"], "cost": 30},
    "Mythic": {"seeds": ["Ember Lily", "Sugar Apple", "Beanstalk", "Kiwi"], "cost": 60}
}
user_lootbox_stock = defaultdict(lambda: {"Common": 2, "Rare": 1, "Mythic": 1})

# --- Shovel ---
# Command /shovel allows user to remove a growing seed instantly

# --- New Seeds ---
seeds.update({"Cauliflour": (20, 0), "Kiwi": (50, 900), "Feijoa": (120, 10000)})
SEED_RARITIES.update({"Cauliflour": "Rare", "Kiwi": "Legendary", "Feijoa": "Divine"})
RARITY_CHANCES["Divine"] = 0.01
mutations["specific"].update({"Feijoa": {"Celestial": {"multiplier": 5.0, "rarity": 0.01, "description": "Blessed by stars", "priority": True}}})

# --- Shop ---
user_shop_stock = defaultdict(list)

# --- Achievements ---
achievement_definitions = {
    "First Harvest": {"condition": lambda u: len(user_inventory[u]["grown"]) >= 1, "reward": "🌱"},
    "Seed Tycoon": {"condition": lambda u: user_sheckles[u] >= 1000, "reward": "💰"},
    "Divine Touch": {"condition": lambda u: any(s.name == "Feijoa" for s in user_inventory[u]["grown"]), "reward": "✨"}
}
...
def gain_xp(uid, amt):
    user_xp[uid] += amt
    if user_xp[uid] >= (user_levels[uid] + 1) * 100:
        user_levels[uid] += 1

# --- Anti-Spam ---
user_last_message_time = defaultdict(lambda: 0)
user_message_burst = defaultdict(int)

@tree.command(name="buy_fertilizer")
@app_commands.describe(fertilizer_type="Type of fertilizer to buy")
@app_commands.choices(fertilizer_type=[
    app_commands.Choice(name="Speed Boost (15 sheckles)", value="Speed Boost"),
    app_commands.Choice(name="Mutation Booster (30 sheckles)", value="Mutation Booster")
])
async def buy_fertilizer(interaction: discord.Interaction, fertilizer_type: app_commands.Choice[str]):
    fertilizer = fertilizer_type.value
    cost = fertilizers[fertilizer]["cost"]
    
    if user_sheckles.get(interaction.user.id, 0) < cost:
        return await interaction.response.send_message("❌ Not enough sheckles!", ephemeral=True)
    
    user_sheckles[interaction.user.id] -= cost
    user_fertilizers[interaction.user.id][fertilizer] += 1
    
    await interaction.response.send_message(
        f"✅ Purchased {fertilizer} fertilizer! Use with `/use_fertilizer`.\n"
        f"{fertilizers[fertilizer]['description']}",
        ephemeral=True
    )

@tree.command(name="use_fertilizer")
@app_commands.describe(fertilizer_type="Type of fertilizer to use", plant="Plant to apply it to")
async def use_fertilizer(interaction: discord.Interaction, fertilizer_type: str, plant: str):
    base, mut, _ = normalize_seed_name(plant)
    user_id = interaction.user.id
    
    if user_fertilizers[user_id].get(fertilizer_type, 0) <= 0:
        return await interaction.response.send_message(f"❌ You don't have any {fertilizer_type} fertilizer!", ephemeral=True)
    
    # Find growing seed
    growing = user_inventory[user_id]["growing"]
    seed = next((s for s in growing if s.name == base and (mut is None or s.mutation == mut)), None)
    
    if not seed:
        return await interaction.response.send_message("❌ No matching growing plant found!", ephemeral=True)
    
    fertilizer = fertilizers[fertilizer_type]
    user_fertilizers[user_id][fertilizer_type] -= 1
    
    if fertilizer["effect"] == "reduce_time":
        time_reduction = (seed.finish_time - time.time()) * fertilizer["amount"]
        seed.finish_time -= time_reduction
        msg = f"⏱️ Growth time reduced by {int(time_reduction)} seconds!"
    elif fertilizer["effect"] == "increase_mutation":
        # For next seed planted
        # This would require modifying your planting logic to check for active boosts
        msg = "✨ Next planted seed will have doubled mutation chance!"
    
    await interaction.response.send_message(f"✅ Applied {fertilizer_type} to {seed.name}! {msg}", ephemeral=True)

@tree.command(name="open_lootbox")
@app_commands.describe(lootbox_type="Type of lootbox to open")
@app_commands.choices(lootbox_type=[
    app_commands.Choice(name="Common (10 sheckles)", value="Common"),
    app_commands.Choice(name="Rare (30 sheckles)", value="Rare"),
    app_commands.Choice(name="Mythic (60 sheckles)", value="Mythic")
])
async def open_lootbox(interaction: discord.Interaction, lootbox_type: app_commands.Choice[str]):
    ltype = lootbox_type.value
    cost = lootbox_types[ltype]["cost"]
    
    if user_lootbox_stock[interaction.user.id][ltype] <= 0:
        return await interaction.response.send_message(f"❌ You don't have any {ltype} lootboxes!", ephemeral=True)
    
    user_lootbox_stock[interaction.user.id][ltype] -= 1
    possible_seeds = lootbox_types[ltype]["seeds"]
    seed_gained = random.choice(possible_seeds)
    
    grow_time = time.time() + random.randint(300, 600)
    seed_obj = GrowingSeed(seed_gained, grow_time)
    user_inventory[interaction.user.id]["growing"].append(seed_obj)
    
    embed = discord.Embed(
        title=f"🎁 {ltype} Lootbox Opened!",
        description=f"You got a **{seed_gained}** seed!",
        color=discord.Color.gold()
    )
    await interaction.response.send_message(embed=embed)

@tree.command(name="shovel")
@app_commands.describe(plant="Plant to remove")
async def shovel(interaction: discord.Interaction, plant: str):
    base, mut, _ = normalize_seed_name(plant)
    user_id = interaction.user.id
    growing = user_inventory[user_id]["growing"]
    ...
    growing.remove(seed)
    await interaction.response.send_message(f"🗑️ Removed {seed.name} from your garden.", ephemeral=True)

@tree.command(name="daily_quest")
async def daily_quest(interaction: discord.Interaction):
    user_id = interaction.user.id
    if user_quests[user_id]["daily_done"]:
        return await interaction.response.send_message("❌ You've already completed your daily quest today!", ephemeral=True)
    
    # Check if completed
    grown_today = len([s for s in user_inventory[user_id]["grown"] if s.finish_time > time.time() - 86400])
    if grown_today >= 1:
        user_quests[user_id]["daily_done"] = True
        user_sheckles[user_id] += daily_quest["reward"]
        await interaction.response.send_message(
            f"✅ Daily quest completed! +{daily_quest['reward']} sheckles",
            ephemeral=True
        )
    else:
        await interaction.response.send_message(
            f"🌱 Daily Quest: {daily_quest['desc']}\n"
            f"Reward: {daily_quest['reward']} sheckles\n"
            f"Progress: {grown_today}/1",
            ephemeral=True
        )

@tree.command(name="weekly_quest")
async def weekly_quest(interaction: discord.Interaction):
    user_id = interaction.user.id
    progress = user_quests[user_id]["weekly_progress"]
    
    if progress >= weekly_quest["target"]:
        user_sheckles[user_id] += weekly_quest["reward"]
        user_quests[user_id]["weekly_progress"] = 0
        return await interaction.response.send_message(
            f"🎉 Weekly quest completed! +{weekly_quest['reward']} sheckles",
            ephemeral=True
        )
    
    await interaction.response.send_message(
        f"📅 Weekly Quest: {weekly_quest['desc']}\n"
        f"Reward: {weekly_quest['reward']} sheckles\n"
        f"Progress: {progress}/{weekly_quest['target']}",
        ephemeral=True
    )

@tree.command(name="announce_event")
async def announce_event(interaction: discord.Interaction):
    ...
    if current_plant_event:
        effect_desc = {
            "delay": f"⚠️ {current_plant_event['name']} event! All plants grow {current_plant_event['delay']}s slower.",
            "speed": f"☀️ {current_plant_event['name']} event! Plants grow {int(100 - (current_plant_event['multiplier'] * 100))}% faster."
        }
        await interaction.response.send_message(effect_desc[current_plant_event["effect"]])
    else:
        await interaction.response.send_message("ℹ️ No active plant events right now.")

@tree.command(name="check_xp")
async def check_xp(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="📊 Your Progress", color=discord.Color.blue())
    embed.add_field(name="Level", value=str(level), inline=True)
    embed.add_field(name="XP", value=f"{xp}/{next_level_xp}", inline=True)
    embed.add_field(name="Progress", value=f"{int((xp/next_level_xp)*100)}%", inline=True)
...
async def on_message(message):
    ...
    # Anti-spam check
    current_time = time.time()
    time_since_last = current_time - user_last_message_time[message.author.id]
    user_last_message_time[message.author.id] = current_time
    
    if time_since_last < 2:  # 2 seconds between messages
        user_message_burst[message.author.id] += 1
        if user_message_burst[message.author.id] > 5:
            return  # Ignore spam
    else:
        user_message_burst[message.author.id] = 0
    
    # XP gain (1 XP per message, max 100 per day)
    if time_since_last > 60:  # Only count if not spamming
        gain_xp(message.author.id, 1)
